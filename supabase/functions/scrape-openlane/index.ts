...
async function fetchWithRetries(url: string, options: any, retries = 3, waitTime = 5000) {
  try {
    ...
    // For dynamic content, try to wait and fetch again if needed
    if (html.includes('<div id="react-root" class=""></div>') || !html.includes('vehicle-card')) {
      console.log("Initial content doesn't have loaded data, waiting for JS rendering...");
...
// Function to extract car details from OpenLane HTML
function extractCarsFromHTML(html: string) {
  try {
    ...
    const vehicleCards = document.querySelectorAll('.vehicle-card, .car-card, .auction-item');
    ...
    if (vehicleCards.length === 0) {
      ...
      const possibleCards = document.querySelectorAll('[class*="car"], [class*="vehicle"], [class*="auction"]');
    ...
    vehicleCards.forEach((card) => {
      try {
        ...
        const titleElement = card.querySelector('h2, h3, .title, .vehicle-title, [class*="title"]');
        const priceElement = card.querySelector('.price, [class*="price"]');
        const imageElement = card.querySelector('img');
        const detailsElements = card.querySelectorAll('.detail, [class*="detail"], .specs, [class*="specs"], p');
        ...
        cars.push({
          ...
          url: url.startsWith('http') ? url : `https://www.openlane.eu${url}`,
          details: details
        });
      } catch (cardError) {
...
serve(async (req) => {
  ...
  try {
    ...
    const html = await fetchWithRetries('https://www.openlane.eu/en/search-results', options, 3, waitTime);
